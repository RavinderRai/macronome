...
from macronome.ai.core.schema import WorkflowSchema
...
class WorkflowValidator:
    ...
    def _validate_dag(self):
        ...
        Checks for cycles and ensuress all nodes are reachable from the start node.
        ...
        if unreachable_nodes:
            raise ValueError(f"Workflow schema contains unreachable nodes: {unreachable_nodes}")

    def _has_cycle(self):
        """Checks if the workflow schema contains a cycle.

        Returns:
            bool: True if the workflow contains a cycle, False otherwise
        """
    ...
    def _validate_connections(self):
        ...
        for node_config in self.workflow_schema.nodes:
            if len(node_config.connections) > 1 and not node_config.is_router:
                raise ValueError(
                    f"Node {node_config.node.__name__} has multiple connections but is not a router."
                )